...
def YcmCorePreload():
    ...
    default_flags = GetDefaultFlags()


def GetDefaultFlags():
    print('initialize default flags')
    flags = [
        '-Wall',
        '-Wextra',
        '-Werror',
    ...
    with open(os.devnull, 'r') as FNULL:
        proc = subprocess.Popen(['c++', '-E', '-x', 'c++', '-', '-v'],
        ...
        for line in proc.stderr:
            if line.startswith("#include"):
                inSearchList = True
            elif line.startswith(" ") and inSearchList:
...
def FlagsForFile(filename):
    ...
    if flags:
        ...
        flags = []
        logging.info("%s: No flags were found !" % (os.path.basename(filename)))
    if not flags:
        logging.info("Going with default flags")
